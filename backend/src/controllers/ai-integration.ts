...
export const generateCharacterDescription = async (req: Request, res: Response) => {
    ...
    const maxTokens = 80;
    const prompt = `Create a short and concise backstory for a Dungeons & Dragons character named ${name} with the following traits: trait: ${trait}, flaw: ${flaw}, bond: ${bond} and ideal: ${ideal}. The backstory should explain how these traits influence the character's personality and actions. The backstory should be concise and naturally fit within approximately ${maxTokens} tokens, without explicitly stating the word count. It should be self-contained, not cut off mid-sentence, and formatted as plain text without bold, italics, or Markdown styling.`;
